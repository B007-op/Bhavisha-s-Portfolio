...
const jumpToFeaturedTarget = card => {
  const filter = card.dataset.featureFilter;
  const targetId = card.dataset.featureTarget;
  if (filter) {
    const matchingTab = document.querySelector(`.work-tab[data-filter="${filter}"]`);
...
document.querySelectorAll('.feature-jump').forEach(card => {
  card.addEventListener('click', () => jumpToFeaturedTarget(card));
  card.addEventListener('keydown', event => {
    if (event.key === 'Enter' || event.key === ' ') {
      event.preventDefault();
      jumpToFeaturedTarget(card);
    }
  });
});
...
const caseStudies = {
  insideinsight: {
    ...
    outcome: 'Surfaced listings priced up to 65% below comparable properties, representing $51,830 in recoverable revenue. The project was named Judges’ & Instructor’s Favorite at the Big Data & AI Trends Market 2026.'
  },
  mativ: {
...
const lifeGalleries = {
  art: [
    { src: 'assets/photos/art-sketch.jpg', alt: 'A black and white landscape sketch by Bhavisha' },
    { src: 'assets/photos/art-sketch-2.jpg', alt: 'A second architectural sketch by Bhavisha' }
  ],
  moments: [
    { src: 'assets/photos/food-find.jpg', alt: 'A favorite café dessert' },
    { src: 'assets/photos/cafe-dessert.jpg', alt: 'A sweet café find' },
    { src: 'assets/photos/cafe-dessert-2.jpg', alt: 'Another favorite dessert moment' },
    { src: 'assets/photos/cute-moment.jpg', alt: 'Bhavisha enjoying a quiet café moment' },
    { src: 'assets/photos/cute-moment-2.jpg', alt: 'A cozy everyday moment' },
    { src: 'assets/photos/cute-moment-3.jpg', alt: 'A colorful travel memory' },
    { src: 'assets/photos/fun-holi.jpg', alt: 'Bhavisha enjoying a colorful celebration by the water' },
    { src: 'assets/photos/fun-light-tunnel.jpg', alt: 'Bhavisha walking through a glowing city light tunnel' },
    { src: 'assets/photos/fun-garden.jpg', alt: 'Bhavisha enjoying a quiet afternoon in a garden' }
  ],
  family: [
    { src: 'assets/photos/family-siblings.png', alt: 'Bhavisha with her mother, brother, and sister' },
    { src: 'assets/photos/family-parents.png', alt: 'Bhavisha’s parents together' },
    { src: 'assets/photos/parents.jpg', alt: 'Bhavisha’s parents sharing a warm family moment' },
    { src: 'assets/photos/graduation-family.jpg', alt: 'Bhavisha with family at graduation' }
  ]
};

document.querySelectorAll('.interactive-life-card').forEach(card => {
  const key = card.dataset.gallery;
  const items = lifeGalleries[key] || [];
  const image = card.querySelector('[data-gallery-image]');
  const button = card.querySelector('.life-image-button');
  let index = 0;
  if (!image || !button || items.length < 2) return;
  button.addEventListener('click', () => {
